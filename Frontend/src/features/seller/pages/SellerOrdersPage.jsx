...
const SellerOrdersPage = () => {
  ...
  const statusCounts = {
    ...
    pending:
      orders?.data?.filter((o) => o.orderId?.status === "pending")
        .length || 0,
    confirmed:
      orders?.data?.filter((o) => o.orderId?.status === "confirmed")
        .length || 0,
    processing:
      orders?.data?.filter((o) => o.orderId?.status === "processing")
        .length || 0,
    shipped:
      orders?.data?.filter((o) => o.orderId?.status === "shipped")
        .length || 0,
    delivered:
      orders?.data?.filter((o) => o.orderId?.status === "delivered")
        .length || 0,
  };
