...
const getStatusColor = (status) => {
  switch (status) {
    case "confirmed": return "text-blue-400 border-blue-400/20 bg-blue-400/5";
    case "shipped": return "text-purple-400 border-purple-400/20 bg-purple-400/5";
    case "delivered": return "text-[#D4AF37] border-[#D4AF37]/20 bg-[#D4AF37]/5";
    case "cancelled": return "text-red-400 border-red-400/10 bg-red-400/5";
...
const SellerOrderCard = ({ order }) => {
  ...
  const availableStatuses = STATUS_OPTIONS.filter(s => statusOrder.indexOf(s) > currentIndex);
        ...
        <div className="flex flex-col lg:flex-row justify-between gap-10">
          ...
          <div className="flex-1 space-y-8">
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <span className={`text-[9px] px-4 py-1.5 rounded-full uppercase tracking-[0.2em] border font-black ${getStatusColor(currentStatus)}`}>
                {currentStatus}
              </span>
            </div>
            ...
            <div className="flex gap-6 items-start">
              ...
              <div className="space-y-1">
                <h4 className="text-[#F5F0E8] font-[Georgia] italic text-lg leading-tight">{order.productName}</h4>
                <p className="text-white/30 text-[10px] uppercase tracking-widest font-bold">
                  Quantity: {order.quantity} <span className="mx-2 text-white/10">|</span> Unit: ₹{order.priceAtPurchase?.toLocaleString()}
                </p>
                <div className="pt-2">
            ...
            {address && (
              <div className="relative p-5 rounded-2xl bg-white/[0.02] border border-white/5 overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none">
                   <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" className="text-white"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                </div>
                <p className="text-[#D4AF37] text-[9px] uppercase tracking-[0.3em] font-black mb-3">Consignee Details</p>
                <p className="text-[#F5F0E8] text-xs font-bold mb-1">{address.name}</p>
                <p className="text-white/40 text-xs leading-relaxed max-w-sm">
                  {address.addressLine}, {address.city}, {address.state} — <span className="text-white/60 font-mono tracking-tighter">{address.pincode}</span>
                </p>
                <p className="text-[#D4AF37] text-[10px] mt-3 font-mono">TEL: {address.phone}</p>
              </div>
            )}
          </div>

          {/* Action Sidebar */}
          {currentStatus !== "delivered" && currentStatus !== "cancelled" && availableStatuses.length > 0 && (
