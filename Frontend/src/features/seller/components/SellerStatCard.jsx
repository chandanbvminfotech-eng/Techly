...
const SellerStatCard = ({ title, value, icon, trend, color = 'gold' }) => {
  ...
  return (
    <div className={`relative group rounded-2xl ${colors.bg} border ${colors.border} backdrop-blur-sm overflow-hidden transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl`}>
      ...
      <div className="relative p-6">
        <div className="flex items-start justify-between mb-4">
          ...
          {trend && (
            <div className={`text-xs font-mono font-bold ${trend >= 0 ? colors.trendUp : colors.trendDown}`}>
              {trend >= 0 ? '+' : ''}{trend}%
            </div>
          )}
        </div>
