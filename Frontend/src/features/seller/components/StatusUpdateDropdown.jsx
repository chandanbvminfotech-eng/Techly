...
const StatusUpdateDropdown = ({ orderId, currentStatus, onStatusChange }) => {
  ...
  const statusFlow = {
    'pending': { next: 'confirmed', label: 'Pending', color: 'text-yellow-400', bg: 'bg-yellow-400/10', border: 'border-yellow-400/20' },
    'confirmed': { next: 'processing', label: 'Confirmed', color: 'text-blue-400', bg: 'bg-blue-400/10', border: 'border-blue-400/20' },
    'processing': { next: 'shipped', label: 'Processing', color: 'text-purple-400', bg: 'bg-purple-400/10', border: 'border-purple-400/20' },
    'shipped': { next: 'delivered', label: 'Shipped', color: 'text-orange-400', bg: 'bg-orange-400/10', border: 'border-orange-400/20' },
    'delivered': { next: null, label: 'Delivered', color: 'text-green-400', bg: 'bg-green-400/10', border: 'border-green-400/20' },
    'cancelled': { next: null, label: 'Cancelled', color: 'text-red-400', bg: 'bg-red-400/10', border: 'border-red-400/20' }
  };

  const availableStatuses = Object.keys(statusFlow).filter(s => 
    s === status || 
