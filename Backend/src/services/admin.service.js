...
const getAdminStats = async() => {
  ...
  const pendingSellerApprovals = await User.countDocuments({ role: "seller", "seller.status": "pending"});
...
const getPendingSeller = async() => {
  const pendingSellers = await User.find({role:"seller","seller.status": "pending"}).select("_id name email");
...
export { getAllUsers, getAllOrders, blockUser, unBlockUser,getAdminStats, getPendingSeller, approveSeller, rejectSeller };
